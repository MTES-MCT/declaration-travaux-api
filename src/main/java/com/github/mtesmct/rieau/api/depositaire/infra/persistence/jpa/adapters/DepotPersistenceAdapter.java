...
public class DepotPersistenceAdapter {
    public JpaDepot toJpa(Depot depot){
        return JpaDepot.builder().id(depot.getId()).type(depot.getType()).etat(depot.getEtat()).date(new Date(depot.getDate().getTime())).build();
    }
    public Depot fromJpa(JpaDepot jpaDepot){
        return new Depot(jpaDepot.getId(), jpaDepot.getType(), jpaDepot.getEtat(), new java.util.Date(jpaDepot.getDate().getTime()));
    }
}
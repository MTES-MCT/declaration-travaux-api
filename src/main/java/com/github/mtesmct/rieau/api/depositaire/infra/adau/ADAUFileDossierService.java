...
public class ADAUFileDossierService implements DossierService {
	...
	public void importerDepot(File file) throws DepotImportException {
		...
		if (type == null)
			throw new DepotImportException(
					"Le type du depot est inconnu: " + code + ". Les seuls types acceptés sont cerfa_13404_DP ou cerfa_13703_DPMI.");
		String id = message.getId();
		if (StringUtils.isEmpty(id))
			throw new DepotImportException(
					"L'id du depot ne peut pas être vide.");
		this.depotRepository.save(new Depot(id, type, this.dateRepository.now()));
	}
